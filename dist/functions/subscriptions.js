...
const subscribeToEvents = (game) => {
            ...
            switch (closestObjectStart) {
                case "waterSource":
                    if (itemString?.startsWith("water")) {
                        ...
                        game.setItem("water", wateringCanImage, context.playerId);
                    break;
                case "snowball":
                    ...
                    if (itemString?.startsWith("snowball")) {
                        ...
                        game.setItem("snowball", snowball, context.playerId);
                    break;
                default: break;
            }
        }
    ...
    let playersHit = [];
    async function throwSnowball(startX, startY, endX, endY, direction) {
        let time = Date.now();
    ...
    function snowBallHit(playerX, playerY, destX, destY, direction, throwLength) {
        if (direction === 1) {
            if (playerX == destX && playerY <= destY && playerY > destY - throwLength)
                return true;
        }
        else if (direction === 3) {
            if (playerX == destX && playerY >= destY && playerY < destY + throwLength)
            ...
            if (playerY == destY && playerX >= destX && playerX < destX + throwLength)
                return true;
        }
        else if (direction === 7) {
            if (playerY == destY && playerX <= destX && playerX > destX - throwLength)
    ...
    async function replaceObj(sequence, imageLink, x, y) {
