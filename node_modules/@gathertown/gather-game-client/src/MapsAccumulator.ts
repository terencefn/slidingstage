...
export class MapsAccumulator {
  readonly partialMaps: { [mapId: string]: Partial<GameMapV2> } = {};
  ...
  readonly completeMaps: { [mapId: string]: GameMapV2 } = {};
  ...
  addChunk(event: ServerClientEventEvent): GameMapV2 | undefined {
    switch (event.$case) {
      ...
      case "mapSetCollisions": {
        const { mapId, x, y, w, h, mask } = event.mapSetCollisions;
        const maskBytes = Buffer.from(mask, "base64");
        const map = this.getMap(mapId);
        // @ts-expect-error Error auto-ignored when enabling TS noUncheckedIndexedAccess. If you're already touching this code, please clean this up while you're at it.
        // TODO: @ENG-4257 Clean these up! See the linear task for more context and advice for cleaning up.
        if (!map.collisions) {
          // assumes first setCollisions is the whole map. maybe a bad assumption
          // @ts-expect-error Error auto-ignored when enabling TS noUncheckedIndexedAccess. If you're already touching this code, please clean this up while you're at it.
          // TODO: @ENG-4257 Clean these up! See the linear task for more context and advice for cleaning up.
        ...
        for (let i = 0; i < h; i++) {
          for (let j = 0; j < w; j++) {
            // @ts-expect-error Error auto-ignored when enabling TS noUncheckedIndexedAccess. If you're already touching this code, please clean this up while you're at it.
            // TODO: @ENG-4257 Clean these up! See the linear task for more context and advice for cleaning up.
            map.collisions[i + y][j + x] = !!maskBytes[i * w + j];
          }
        }
        break;
      }
      ...
      case "mapSetDesks": {
        const { mapId, desks } = event.mapSetDesks;
        const map = this.getMap(mapId);
        // @ts-expect-error Error auto-ignored when enabling TS noUncheckedIndexedAccess. If you're already touching this code, please clean this up while you're at it.
        // TODO: @ENG-4257 Clean these up! See the linear task for more context and advice for cleaning up.
        map.desks = convertDBDesksToDesks(desks, map.dimensions);
        break;
      }
