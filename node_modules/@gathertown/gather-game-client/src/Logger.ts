...
class Logger {
  ...
  // disabling because context really can just be anything, and we send it right to newrelic
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  additionalErrorHandler?: (message: string, context: any) => void;
  // disabling because context really can just be anything, and we send it right to newrelic
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  context: any = {};
  ...
  // @ts-ignore would make it type Error, but this is also useful for printing other complicated objects
  ...
  // disabling because context really can just be anything, and we send it right to newrelic
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  addContext(field: string, value: any) {
