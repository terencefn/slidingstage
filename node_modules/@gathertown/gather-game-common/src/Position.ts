...
export function getCoordsFromCoordsMap(coordsMap: boolean[][]): Position[] {
  return coordsMap.reduce((prev: Position[], curr: boolean[], y) => {
    const validCoords = curr
      .map((val, x) => (val ? { x, y } : null))
      .filter((val): val is Position => !!val);
    return [...prev, ...validCoords];
  }, []);
}

export function getBoundingBox(coords: Position[]): {
  x: number;
  y: number;
  width: number;
  height: number;
...
export function getPosFromBoundingBox(
  bbox: BoundingBox,
  width = Infinity,
  height = Infinity,
): Position[] {
