...
import { Announcer, AreaPosition, Asset, DBDesk, InteractionEnum_ENUM as Interaction, NookDiff, Point, Portal, Space, SpawnPoint, TutorialTaskMapArea, WireArea, WireObject, WireTutorialTasks } from "./generated_DO_NOT_TOUCH/events";
export { Interaction };
export declare type CoordsMap<T> = {
    [y: number]: {
        [x: number]: T;
    };
};
...
export interface GameMapDB extends Omit<GameMapBaseDoNotUseDirectly, "walls" | "floors" | "areas"> {
    walls?: WallFloorDB;
    floors?: WallFloorDB;
    areas?: DBAreas;
    desks?: never;
...
export interface GameMapHttpPublic extends Omit<GameMapDB, "spaces" | "typeCheckFakeField" | "desks"> {
    spaces?: Space[];
    desks?: Desks;
    typeCheckFakeField?: never;
}
export declare type TileEffect = Portal | Space | SpawnPoint | Announcer;
...
declare type NookRequiredFields = "nookCoords" | "name";
export declare type Nook = MakeFieldsRequired<NookDiff, NookRequiredFields>;
...
export declare function coordsMapToPositions(coordsMap: CoordsMap<true>): Position[];
